...
def EV_F_ITER(X, k_init, n_agents, grid_list, theta):
    ...
    VT_sum=utility(cons, lab) + beta*sum((grid.evaluate(knext) for grid in grid_list))/ntheta       
    return VT_sum
...
def EV_GRAD_F_ITER(X, k_init, n_agents, grid_list, theta):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h            
            fx2=EV_F_ITER(xAdj, k_init, n_agents, grid_list, theta)
            
            xAdj[ixN]=X[ixN] - h
            fx1=EV_F_ITER(xAdj, k_init, n_agents, grid_list, theta)
...
def EV_G_ITER(X, k_init, n_agents, theta):
    ...
    f_prod=theta*output_f(k_init, lab)
    Gamma_adjust=0.5*zeta*k_init*((inv/k_init - delta)**2.0)
    sectors_sum=cons + inv - delta*k_init - (f_prod - Gamma_adjust)
    G[3*n_agents]=np.sum(sectors_sum)
